...
7. Update `/project-config` and `project-config-authoring` to invoke `npx container-superposition defaults --json` before direct home-file fallback. The CLI output informs authoring only; accepted values must become explicit repository shared/local intent before they can affect replay.
...
- Help/contract checks: `npx container-superposition defaults --help`; run `defaults --json` with isolated HOME fixtures for no file, each filename, both filenames, invalid selected input, and variable-like literals. Confirm stdout parses as JSON and workspace/home hashes are unchanged.
...
- Updated `/project-config` and `/skill:project-config-authoring` to start from `npx container-superposition defaults --json`, with direct home-file inspection only as an unavailable-command fallback, and retained bootstrap-only/non-replay guidance.
