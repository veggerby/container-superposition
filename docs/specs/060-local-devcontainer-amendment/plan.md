...
- A generated alternate config whose basename is always `devcontainer.json` for Dev Container CLI compatibility: `superposition-local/devcontainer.json` beside a `devcontainer.json` base, or `.container-superposition/amendment/devcontainer.json` for a root `.devcontainer.json` base. Because the alternate config is not always in the base file's directory, generation rewrites base-relative references that would otherwise change meaning from the alternate location: `build.dockerfile`, `build.context`, `dockerComposeFile`, and local Dev Container Feature source keys such as `./features/example` or `../shared-feature`. Remote Feature IDs (for example `ghcr.io/devcontainers/features/node:1`), URL Feature sources, and absolute Feature source paths are not rewritten.
...
4. Image-based and Dockerfile/build-based devcontainers are supported because generation rewrites relative `build` paths when needed so the alternate config keeps the same base semantics from its new location.
...
- **Simplicity/proportionality — ALIGNED:** a sibling alternate config plus final compose override uses standard devcontainer/compose capabilities and avoids copying a whole devcontainer tree or building a proprietary runtime. The implementation performs only the bounded base-relative rewrites required when the alternate config's directory differs from the team-owned base: build paths, compose file paths, and local Dev Container Feature source keys.
...
- **Relative-path risk:** the alternate config may live below a sibling `superposition-local/` directory, so generation must rewrite base-relative build paths, compose file references, and local Dev Container Feature source keys while preserving remote/URL/absolute Feature sources unchanged. Tests must cover Dockerfile context, compose files inside/outside the devcontainer directory, and local Feature source keys such as `./features/example` and `../shared-feature`.
...
- Bounded defect fix for Dev Container CLI 0.82.0 changed the generated alternate config from `devcontainer.superposition-local.json` / `.devcontainer.superposition-local.json` to a local `devcontainer.json` path so `devcontainer --config` accepts it; relative base path semantics are preserved by rewriting base-relative build paths, compose references, and local Dev Container Feature source keys in the generated alternate config.
