...
- [ ] T022 [US2] Apply project-config values as default persisted input for standard init without prompting for already-declared values in `scripts/init.ts`
- [ ] T023 [US2] Preserve run-scoped CLI override precedence over project-config defaults in `scripts/init.ts`
- [ ] T024 [US2] Keep explicit manifest mode isolated from repository project-config defaults in `scripts/init.ts`
- [ ] T025 [US2] Ensure manifest-only generation paths ignore project-config defaults when `--from-manifest` is explicit in `tool/__tests__/manifest-only.test.ts`
...
- [ ] T026 [US3] Add command regression coverage for invalid YAML, unsupported keys, and unsupported values in `tool/__tests__/commands.test.ts`
- [ ] T027 [US3] Add regression coverage for conflicting selections, missing required non-interactive values, and dual-file ambiguity in `tool/__tests__/commands.test.ts`
- [ ] T028 [US3] Add regression coverage for unsupported customization declarations outside the clean-generation surface in `tool/__tests__/commands.test.ts`

### Implementation for User Story 3

- [ ] T029 [US3] Validate project-config syntax, supported keys, supported values, and conflicting selections before generation in `tool/schema/project-config.ts`
- [ ] T030 [US3] Report repository-root ambiguity and non-interactive missing-value failures with actionable guidance in `scripts/init.ts`
- [ ] T031 [US3] Reject unsupported customization declarations that fall outside the supported clean-generation surface in `tool/schema/project-config.ts`
...
- [ ] T032 [P] Document project-config usage, parity scope, and supported customization examples in `README.md`
- [ ] T033 [P] Document local and CI workflow examples for project config in `docs/workflows.md`
- [ ] T034 [P] Document project-config parity examples and customization cases in `docs/examples.md`
- [ ] T035 Update the user-visible project-config change summary in `CHANGELOG.md`
- [ ] T036 Record final verification steps and observed outcomes in `docs/specs/002-superposition-config-file/quickstart.md`
- [ ] T037 Run `npm test`, `npm run lint`, and `npm run test:smoke`, then record the results in `docs/specs/002-superposition-config-file/quickstart.md`
...
- `T032`, `T033`, and `T034` can run in parallel after behavior is stable
