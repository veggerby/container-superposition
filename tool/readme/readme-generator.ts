...
function extractSectionSummary(section: MarkdownSection): string {
    ...
    if (section.title.toLowerCase().includes('connection')) {
        ...
        for (const line of lines) {
            ...
            if (
                trimmed.match(
                    /^(#\s*)?(Hostname|Host|Port|Database|DB|User|Username|Password|URL|Connection):/i
                )
            ) {
                const cleaned = trimmed
                    .replace(/^#\s*/, '')
                    .replace('Connection string', 'Connection URL');
        ...
        for (const subsection of section.subsections.slice(0, 4)) {
            if (subsection.title) {
                ...
                if (subsection.content) {
                    const firstLine = subsection.content
                        .split('\n')
...
function generateHeader(answers: QuestionnaireAnswers): string {
    ...
    parts.push('');
    parts.push(
        `> Generated by Container Superposition on ${new Date().toISOString().split('T')[0]}`
    );
