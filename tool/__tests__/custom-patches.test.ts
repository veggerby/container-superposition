...
describe('Custom Patches', () => {
    ...
    it('should load custom docker-compose.patch.yml', async () => {
        ...
        fs.writeFileSync(path.join(customDir, 'docker-compose.patch.yml'), yaml.dump(patch));

        const customPatches = loadCustomPatches(TEST_OUTPUT_DIR);
        expect(customPatches).not.toBeNull();
        expect(customPatches?.dockerComposePatch?.services?.['custom-service']).toBeDefined();
        expect(customPatches?.dockerComposePatch?.services?.['custom-service'].image).toBe(
            'myorg/custom:latest'
        );
    });
...
`;
        ...
        expect(customPatches?.environmentVars?.DATABASE_URL).toBe(
    ...
    it('should detect custom scripts', async () => {
        ...
        expect(customPatches?.scripts?.postCreate?.[0]).toBe(
            `bash ${outputDirName}/custom/scripts/post-create.sh`
        );
        expect(customPatches?.scripts?.postStart?.[0]).toBe(
            `bash ${outputDirName}/custom/scripts/post-start.sh`
        );
    });
    ...
    it('should apply custom environment variables', async () => {
        ...
        fs.writeFileSync(path.join(customDir, 'environment.env'), 'MY_CUSTOM_VAR=custom-value\n');
    ...
    it('should add custom lifecycle scripts to devcontainer', async () => {
        ...
        const postCreateCommands = Object.values(
            devcontainer.postCreateCommand as Record<string, string>
        );
        expect(
            postCreateCommands.some((cmd: string) => cmd.includes('custom/scripts/post-create.sh'))
        ).toBe(true);

        // Check postStartCommand includes custom script
        expect(devcontainer.postStartCommand).toBeDefined();
        const postStartCommands = Object.values(
            devcontainer.postStartCommand as Record<string, string>
        );
        expect(
            postStartCommands.some((cmd: string) => cmd.includes('custom/scripts/post-start.sh'))
        ).toBe(true);
    });
});
