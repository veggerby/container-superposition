...
describe('Overlay Loader', () => {
  ...
  describe('loadBaseImages', () => {
    ...
    it('should include package manager for each image', () => {
      ...
      for (const image of baseImages) {
        if (image.id !== 'custom') { // custom may not have package_manager
  ...
  describe('loadOverlaysConfig', () => {
    it('should build complete config from manifests', () => {
      ...
      expect(config.language_overlays.length).toBeGreaterThan(0);
      expect(config.database_overlays.length).toBeGreaterThan(0);
    ...
    it('should correctly categorize overlays', () => {
      ...
      // Language overlays
      expect(config.language_overlays.some(o => o.id === 'nodejs')).toBe(true);
      expect(config.language_overlays.some(o => o.id === 'python')).toBe(true);
      
      // Database overlays
      expect(config.database_overlays.some(o => o.id === 'postgres')).toBe(true);
      expect(config.database_overlays.some(o => o.id === 'redis')).toBe(true);
      
      // Observability overlays
      expect(config.observability_overlays.some(o => o.id === 'prometheus')).toBe(true);
      expect(config.observability_overlays.some(o => o.id === 'grafana')).toBe(true);
      
      // Cloud tools
      expect(config.cloud_tool_overlays.some(o => o.id === 'aws-cli')).toBe(true);
      expect(config.cloud_tool_overlays.some(o => o.id === 'kubectl-helm')).toBe(true);
      
      // Dev tools
      expect(config.dev_tool_overlays.some(o => o.id === 'docker-sock')).toBe(true);
      expect(config.dev_tool_overlays.some(o => o.id === 'playwright')).toBe(true);
    });
    ...
    it('should sort overlays by order then name', () => {
      ...
      const obsOverlays = config.observability_overlays;
    ...
    it('should validate required fields in all overlays', () => {
      ...
      for (const overlay of allOverlays) {
        ...
        expect(['language', 'database', 'observability', 'cloud', 'dev', 'preset']).toContain(overlay.category);
      }
    });
  });
