...
/**
 * Handle Docker Compose file merging
 */
function handleDockerCompose(outputPath: string, overlays: string[]): void {
  const composeFiles: string[] = [];
  
  for (const overlay of overlays) {
    const composePath = path.join(OVERLAYS_DIR, overlay, 'docker-compose.yml');
    if (fs.existsSync(composePath)) {
      composeFiles.push(overlay);
    }
  }
  ...
  // Simple approach: copy each compose file with a prefix
  for (const overlay of composeFiles) {
    const src = path.join(OVERLAYS_DIR, overlay, 'docker-compose.yml');
    const dest = path.join(outputPath, `docker-compose.${overlay}.yml`);
    fs.copyFileSync(src, dest);
  }
  
  console.log(chalk.dim(`   📋 Created ${composeFiles.length} docker-compose file(s)`));
}
...
export async function composeDevContainer(answers: QuestionnaireAnswers): Promise<void> {
  ...
  // 8. Handle Docker Compose files
  handleDockerCompose(outputPath, overlays);
  
  // 9. Update docker-compose reference in devcontainer.json if needed
