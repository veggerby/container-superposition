...
function createRegistryFiles(config: OverlaysConfig): void {
  ...
  const baseImagesContent = yaml.dump({ base_images: config.base_images }, {
    indent: 2,
    lineWidth: 120,
    noRefs: true,
  });
  ...
  const baseTemplatesContent = yaml.dump({ base_templates: config.base_templates }, {
    indent: 2,
    lineWidth: 120,
    noRefs: true,
  });
    ...
    for (const overlay of category.overlays) {
      ...
      try {
        ...
        if (manifest.id !== overlay.id) {
          console.log(chalk.red(`  ✗ ID mismatch in ${overlay.id}: manifest has '${manifest.id}'`));
          errors++;
        }
        ...
        if (manifest.conflicts && manifest.conflicts.length > 0) {
          for (const conflictId of manifest.conflicts) {
            ...
            if (fs.existsSync(conflictManifestPath)) {
              ...
              if (!conflictManifest.conflicts || !conflictManifest.conflicts.includes(overlay.id)) {
                console.log(chalk.yellow(`  ⚠️  Non-bidirectional conflict: ${overlay.id} conflicts with ${conflictId}, but not vice versa`));
                warnings++;
              }
            }
          }
