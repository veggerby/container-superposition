...
- **`cross-distro-packages` feature deleted on regen** — When `cleanupStaleDirFiles` was introduced to remove stale scripts within registered directories, it also recurse into the `features/` directory and deleted `cross-distro-packages/devcontainer-feature.json` and `install.sh` because those files were never added to the file registry. The composer now registers every file in the copied feature directory, so `devcontainer build` can locate the local feature.
